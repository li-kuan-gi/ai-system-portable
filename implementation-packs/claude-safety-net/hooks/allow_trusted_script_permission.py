...
def evaluate_trusted(command: str, payload_cwd: Path) -> bool:
    ...
    B 路線：不含任何「會跑到沙箱外」的成分（git/kubectl/gh/docker/helm/argocd/trusted-script）
    → 整條留在沙箱內，由沙箱圍堵（FS 邊界只可寫 .worktrees/tmp、無網路、seccomp 擋 unix socket、
    denyRead 憑證）→ 廣義放行（for 迴圈、$()、find、python、任意讀取皆可，殘餘風險僅
    .worktrees/tmp 內可復原破壞）。背景 & / nohup 等已於 main 的 detached 階段 deny。

    含「會跑到盒外」成分 → 走嚴格逐段驗證：trusted-script / 唯讀 git / 安全讀取，盒外不准
    explore(find/python)，讀取運算元限 ws/tmp（擋盒外讀憑證），$()/不安全重導/`| sh` 擋。"""
    ...
    # B 路線：整條留在沙箱內 → 由沙箱圍堵 → 廣義放行。
    if not contains_excluded_tool(remainder):
        return True
    ...
    # 純讀取/探索鏈（無 excluded 段）留在沙箱內，沙箱兜底 → 准 read/explore，不需 anchor。
    return all(k in ("read", "explore") for k in kinds)
